...
### 4주차 — 마감·확산
- [x] 3D 갤러리·PDF 포트폴리오를 새 모듈 산출물과 연결 — 발표 탭을 3모드(슬라이드쇼/3D 갤러리/PDF 포트폴리오)로 확장. 3D 갤러리=공개 AI 작품+작품 평가 사진(출석번호만 표시, 클릭 확대), PDF=학생별 전 모듈 산출물 1부(감상 1·2차+펠드만, 복원 1·2차+이미지, AI 작품, 인물의 하루, 스토리보드, 작품 평가+다짐). three.js·jspdf는 lazy 청크 분리(학생 번들 보호)
- [x] 외부 링크 섹션 (유니콘 공용 / 유니콘 이미지 생성 교사 전용) — `src/data/externalLinks.js` 단일 관리. 교사 대시보드 왼쪽 열 '🔗 외부 도구' 카드(교사 전용 배지), 학생 세션 헤더에 유니콘 링크 버튼(새 탭)
...
### 2026-07-24 (4주차)
- 4주차 ① 완료 (3D 갤러리·PDF 포트폴리오 연결):
  - 고아 상태였던 `Gallery3D.jsx`·`pdfService.js`(구 접속 체계 삭제 때 연결 끊김)를 새 모듈 산출물에 재연결
  - 교사 대시보드 발표 탭 → 3모드: 🎬 슬라이드쇼(기존) / 🏛️ 3D 갤러리 / 📄 PDF 포트폴리오
  - `portfolioService.js` 신설: 세션의 deepAppreciations·restoreChallenges·videoPrompts(인물/스토리보드)·artworkReviews·generationQueue(published)를 교사 권한으로 읽어 출석번호별 묶음 + 갤러리 목록 생성. 복원 챌린지 queueId → 공개 이미지 매핑
  - 3D 갤러리: 공개 AI 작품 + 작품 평가 업로드 사진 전시(제목=출석번호만), 작품 수에 맞춰 벽 확장·2줄 배치(기존엔 5번째 작품부터 바닥 아래로 뚫림), 클릭 시 확대 모달
  - PDF: 학생별 활동 전체 산출물 1부(A4) — 감상 1·2차+AI 질문+펠드만 단계(교사 확정 우선), 복원 1·2차 관찰+이미지+다른 점+성찰, AI 작품, 인물의 하루 관찰+프롬프트+영상 URL, 스토리보드 컷, 작품 평가 사진+루브릭 피드백+교사 한마디+성장 다짐. 한글은 기존 canvas 텍스트 이미지 방식 재사용
  - 번들 최적화: three.js(940KB)·jspdf(364KB)를 lazy import로 분리 — 메인 번들 864KB 유지(3D/PDF 열 때만 다운로드), 학생 크롬북 로딩 보호
  - 갤러리·PDF 모드 진입은 useEffect로 구동(활동 전환 시 무한 로딩 버그 수정), 학급 전환 시 발표 탭 상태 초기화
- **다음 작업**: 4주차 ② 외부 링크 섹션 (유니콘 공용 / 유니콘 이미지 생성 교사 전용)
- 4주차 ② 완료 (외부 링크 섹션): `externalLinks.js`에 URL 단일 관리. 교사 대시보드 = 유니콘 + 유니콘 이미지 생성('교사 전용' 배지), 학생 세션 헤더 = 유니콘만(공용, 새 탭). KLING/FLOW 링크는 기존 오퍼레이터 보드에 이미 있음
- **다음 작업**: 4주차 ③ 파비콘 + OG 이미지 제작
- 4주차 ③ 완료 (파비콘+OG): 점 3개(봄·봄·봄) 모티프 favicon.svg + apple-touch-icon.png + og.png(1200×630, PowerShell System.Drawing로 생성 — ps1 한글은 UTF-8 BOM 필수). index.html 제목·설명·OG 메타 정비. 플랫폼 이름은 가칭 '봄·봄·봄 스튜디오' 사용 — **사용자 확정 대기(미결 항목)**
- **다음 작업**: 4주차 ④ 전자칠판·크롬북 화면 점검 + 키보드 UX
