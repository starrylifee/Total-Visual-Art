...
- 4주차 ④ 진행 중 (중단, 다음 세션에서 재개):
  - 키보드 UX 점검 완료: 학생 쪽(/join 단계별 포커스+엔터, 각 모듈 autoFocus, 챗봇 엔터)은 이미 잘 돼 있음. 교사 쪽 보완 커밋 — 새 학급 모달(autoFocus+엔터 생성), 새 활동 모달(제목 autoFocus)
  - 화면 점검용 테스트 데이터 생성됨: 학급 '[화면점검용] 삭제 예정'(classId 52odu82AWboqaCdzxVmU), 활동 '[점검] 전 모듈 화면 점검'(전 features on, 활동코드 **5QZ4MQ**) — 점검 끝나면 joinCodes/5QZ4MQ + 학급 문서 삭제 필요 (생성 스크립트: 스크래치패드 make-test-session.mjs, firebase-admin은 프로젝트 루트에서 실행)
  - 브라우저 점검 시작 단계에서 중단. /join 화면은 확인함: 큰 글씨·한 화면·포커스 OK. **발견 사항**: 기본 테마가 핑크 '우리 반 미술 교실'(구 명칭) — 사용자 지침(모던·포멀, AI 귀여움 배제)과 충돌 + OG 제목 '봄·봄·봄 스튜디오'와 불일치. 학생 화면 헤더에 '로그인' 버튼 노출됨. 재개 시: 테마 기본값·헤더 명칭 정리 여부 사용자와 상의 → 학생 세션 전 탭(감상/복원/인물/스토리보드/작품평가) 16:9 캡처 점검 → 교사 화면(칠판 모드·감상 현황·슬라이드쇼) 점검 → 글씨·레이아웃 보정

### 2026-07-25 (4주차 ④ 완료)
- 사용자 결정: 명칭 '봄·봄·봄 스튜디오' 통일 + 기본 테마 '봄봄봄 모던'(인디고, 기존 테마는 선택지 유지) + 학생 화면(/join, /student/*) 로그인 버튼 숨김 (커밋 55b6bea)
- **운영 사고 해결**: Vercel 도메인에서 교사 구글 로그인 불가(auth/unauthorized-domain) — Firebase 승인 도메인이 localhost·firebaseapp·web.app뿐이었음. 사용자 승인 후 Identity Toolkit Admin API(서비스 계정)로 total-visual-art.vercel.app 추가. **새 도메인으로 이전하면 Firebase 승인 도메인부터 확인할 것**
- 화면 점검(1366×768): 학생 입장 3단계·전 탭, 교사 대시보드·칠판 모드·감상 현황(30명 그리드 한 화면)·발표 3모드 — 실기기 해상도에서 스크롤·글씨 문제 없음
- 점검 중 발견·수리한 실버그 3건:
  1. 3D 갤러리/PDF 산출물 로드가 영원히 '불러오는 중' — useEffect가 자기 setState로 재실행되며 이전 실행을 취소하는 구조. ref 가드로 교체 (6e6d0be)
  2. 3D 갤러리 이미지 1장 로드 실패 시 **대시보드 전체 흰 화면 크래시** — drei 텍스처 예외에 에러 바운더리 없음. 프레임 단위 격리(실패 프레임만 회색) (ae18a79). 참고: 위키미디어 Special:FilePath는 CORS 헤더가 없어 3D 텍스처 로드 불가(실전 갤러리 데이터는 전부 base64라 무관)
  3. 포트폴리오 PDF 9.1MB → 449KB — 텍스트 블록을 투명 PNG로 넣던 것을 흰 배경 JPEG로 전환 + 이미지 실패 시 '(생략)' 안내 (23433bd)
  - 부수 개선: 인물 사진 깨질 때 학생 안내 + 활동 생성 모달 사진 미리보기 (df85faf)
- PDF 실검증: 운영에서 1번 학생 PDF 저장 → 감상 1·2차+AI 질문+펠드만(AI 초벌), AI 그림, 작품 평가 사진+루브릭+교사 한마디+성장 다짐 전부 정상 렌더(한글 포함)
- 테스트 데이터 정리 완료: '[화면점검용]' 학급 recursiveDelete + joinCodes/5QZ4MQ 삭제 — 운영에 점검 잔재 없음
- **다음 작업**: 4주차 ⑤ 앱뜰 등록 → ⑥ 연구 평가 모듈(사전·사후 5문항 + AI 초벌 채점 + 교사 확정 + CSV, 대조군 학급 지원)

### 2026-07-25 (4주차 ⑥ 연구 평가 모듈 완료)
- 구현(커밋 fd516c6): 활동 1개 안에서 사전/사후 단계 전환(학생 화면 15초 폴링으로 따라감), 학생은 한 문항씩 응시·임시 저장·재입장 복원·제출 후 잠금, 교사는 응시 현황 그리드 + AI 초벌 채점(학생당 5문항 1회 호출, 1.2초 간격 순차) → 0~3점 확정 → 다시 풀게 하기, CSV(사전·사후·향상도 + 문항별 점수 + 서술 원문, UTF-8 BOM)
